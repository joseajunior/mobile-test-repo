...
*** Test Cases ***
Validate Add, Update and Delete of Hubometer Prompt to a Card on EFS
    [Tags]    PI:15  JIRA:O5SA-532  qTest:117833782
    [Documentation]  Validate the addition, update and delete of a hubometer prompt into a card
...
Validate Add, Update and Delete of Odometer Prompt to a Card on EFS
    [Tags]    PI:15  JIRA:O5SA-532  qTest:117833782
    [Documentation]  Validate the addition, update and delete of a odometer prompt into a card
    [Setup]  Setting Up The Variables To The Test  EFS
    Verify or Open Account Manager
    Navigate To The Prompt List
...
Validate Add, Update and Delete of Hubometer Prompt to a Card on Irving
    [Tags]    PI:15  JIRA:O5SA-532  qTest:117833782
    [Documentation]  Validate the addition, update and delete of a hubometer prompt into a card
...
Validate Add, Update and Delete of Odometer Prompt to a Card on Irving
    [Tags]    PI:15  JIRA:O5SA-532  qTest:117833782
    [Documentation]  Validate the addition, update and delete of a odometer prompt into a card
    [Setup]  Setting Up The Variables To The Test  Irving
    Verify or Open Account Manager
...
Validate Add, Update and Delete of Hubometer Prompt to a Card on Shell
    [Tags]    PI:15  JIRA:O5SA-532  qTest:117833782
    [Documentation]  Validate the addition, update and delete of a hubometer prompt into a card
...
Validate Add, Update and Delete of Odometer Prompt to a Card on Shell
    [Tags]    PI:15  JIRA:O5SA-532  qTest:117833782
    [Documentation]  Validate the addition, update and delete of a odometer prompt into a card
    [Setup]  Setting Up The Variables To The Test  Shell
    Verify or Open Account Manager
...
Validate That The Fields Does Not Accept Negative Numbers
    [Tags]    PI:15  JIRA:O5SA-532  qTest:117850895
...
Get Data To Be Used During Tests
    [Documentation]    Get Carrier and Card to use in the tests in EFS, Irving and Shell
    ${query}  Catenate  select c.carrier_id, trim(c.card_num) as card_num
                   ...
                   ...  where c.status = 'A' and con.status = 'A'
                   ...  and c.expiredate > TODAY and upper(c.card_num) = lower(c.card_num)
                   ...  and (select count(*) from card_inf ci where info_id = 'ODRD' and info_id = 'HBRD' and ci.card_num = c.card_num) = 0
    ...
    ${len}  Get Length    ${temp}[card_num]
    ${len}  Evaluate    random.randint(0, $len-1)
    &{irvingResult}  Create Dictionary  carrier_id=${temp}[carrier_id][${len}]    card_num=${temp}[card_num][${len}]
    ...
    ${len}  Get Length    ${temp}[card_num]
    ${len}  Evaluate    random.randint(0, $len-1)
    &{shellResult}  Create Dictionary  carrier_id=${temp}[carrier_id][${len}]    card_num=${temp}[card_num][${len}]
...
Setting Up The Variables To The Test
    [Documentation]  Set the variables of carrier_id and card_num based in the instance to be used during test
    [Arguments]    ${instance}
    IF  '${instance.upper()}'=='EFS'
        Set Test Variable  ${carrier_id}  ${efsResult}[carrier_id]
        Set Test Variable  ${card_num}  ${efsResult}[card_num]
    ELSE IF  '${instance.upper()}'=='IRVING'
        Set Test Variable  ${carrier_id}  ${irvingResult}[carrier_id]
        Set Test Variable  ${card_num}  ${irvingResult}[card_num]
    ELSE IF  '${instance.upper()}'=='SHELL'
        Set Test Variable  ${carrier_id}  ${shellResult}[carrier_id]
        Set Test Variable  ${card_num}  ${shellResult}[card_num]
    END
...
Verify Page Shown Values
    ...
    IF  '${distanceCounter.upper()}'=='HUBOMETER'  #set the xpath of the field which stores the data entried during the test
        ${xpath}  Set Variable    //td[text()='CARD']/../td/button[contains(text(), 'Hubometer')]/../../td[contains(text(), 'M:')]
    ELSE IF  '${distanceCounter.upper()}'=='ODOMETER'
        ${xpath}  Set Variable    //td[text()='CARD']/../td/button[contains(text(), 'Odometer')]/../../td[contains(text(), 'M:')]
    END
...
Error Validation
    ...
    ELSE IF  '${fieldName}'=='MESSAGE'
        Wait Until Page Contains Element    //*[@id='cardPromptsAddUpdateMessages']/ul[3]/li
        ${message}  Get Text  //*[@id='cardPromptsAddUpdateMessages']/ul[3]/li
    END
    ...
    ELSE IF  '${expectedError}'=='LABEL'
        Should Be Equal As Strings    ${message}  Add Successful. updatePromptForPolicy ERROR  #Known issue, the message say Successful, but the info isn't added to the DB and there is a explicit error in the screen
    END
