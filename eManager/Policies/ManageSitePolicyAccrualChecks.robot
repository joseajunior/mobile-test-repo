...
*** Test Cases ***
Validate Add, Update and Delete of Hubometer Prompt to a Site Policy on EFS
    [Tags]    PI:15  JIRA:O5SA-532  qTest:117838299
    [Documentation]  Validate the addition, update and delete of a hubometer prompt into a site policy
...
Validate Add, Update and Delete of Odometer Prompt to a Site Policy on EFS
    [Tags]    PI:15  JIRA:O5SA-532  qTest:117838299
    [Documentation]  Validate the addition, update and delete of a odometer prompt into a site policy
    [Setup]  Setting Up The Variables To The Test  EFS
    Verify or Open Account Manager
    Navigate To The Prompt List
...
Validate Add, Update and Delete of Hubometer Prompt to a Site Policy on Irving
    [Tags]    PI:15  JIRA:O5SA-532  qTest:117838299
    [Documentation]  Validate the addition, update and delete of a hubometer prompt into a site policy
...
Validate Add, Update and Delete of Odometer Prompt to a Site Policy on Irving
    [Tags]    PI:15  JIRA:O5SA-532  qTest:117838299
    [Documentation]  Validate the addition, update and delete of a odometer prompt into a site policy
    [Setup]  Setting Up The Variables To The Test  Irving
    Verify or Open Account Manager
...
Validate Add, Update and Delete of Hubometer Prompt to a Site Policy on Shell
    [Tags]    PI:15  JIRA:O5SA-532  qTest:117838299
    [Documentation]  Validate the addition, update and delete of a hubometer prompt into a site policy
...
Validate Add, Update and Delete of Odometer Prompt to a Site Policy on Shell
    [Tags]    PI:15  JIRA:O5SA-532  qTest:117838299
    [Documentation]  Validate the addition, update and delete of a odometer prompt into a site policy
    [Setup]  Setting Up The Variables To The Test  Shell
    Verify or Open Account Manager
...
Validate That The Fields Does Not Accept Negative Numbers
    [Tags]    PI:15  JIRA:O5SA-532  qTest:117867236
...
Get Data To Be Used During Tests
    [Documentation]    Get Carrier and Site Policy to use in the tests in EFS, Irving and Shell
    ${query}  Catenate  select dc.id, dc.ipolicy from def_card dc
                   ...
                   ...  where (select count(*) from def_info di where (di.info_id = 'ODRD' or di.info_id = 'HBRD') and di.ipolicy between 501 and 599 and di.carrier_id = co.carrier_id) = 0
                   ...  and co.status = 'A' and dc.ipolicy between 501 and 599 group by dc.id, dc.ipolicy order by dc.id asc limit 500;
    ...
    ${len}  Get Length    ${temp}[id]
    ${len}  Evaluate    random.randint(0, $len-1)
    &{irvingResult}  Create Dictionary  carrier_id=${temp}[id][${len}]    ipolicy=${temp}[ipolicy][${len}]
    ...
    ${len}  Get Length    ${temp}[id]
    ${len}  Evaluate    random.randint(0, $len-1)
    &{shellResult}  Create Dictionary  carrier_id=${temp}[id][${len}]    ipolicy=${temp}[ipolicy][${len}]
...
Navigate To The Prompt List
    ...
    Wait Until Element Is Visible    //*[@id='SitePolicies']  #click the site policies tab
    Click Element    //*[@id='SitePolicies']
    ...
    Wait Until Element Is Visible    //*[@id='customerSitePoliciesSearchContainer']//button[contains(text(),'Submit')]  #click the submit button
    Click Button    //*[@id='customerSitePoliciesSearchContainer']//button[contains(text(),'Submit')]
