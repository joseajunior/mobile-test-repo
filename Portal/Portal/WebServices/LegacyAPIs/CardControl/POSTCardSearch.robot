...
Documentation  send POST request to search for a card
Force Tags  API  Mobile

*** Variables ***
${login}  /efs-json/user/login
${search}  /efs-json/card/search
${session_alias}  eManager

*** Test Cases ***
Successfully Search For A Card
    [Documentation]  send POST request to search for a card
    [Tags]  PI:15  JIRA:OS5A-505  qTest:117506826
    [Setup]  Find A Random Card Number And Pin Number To Be Used In The API Request
    Log In To Mobile App
    Call The API To Search For A Card Accepted
    Compare Database Values With Card Search JSON Response
...
Compare Database Values With ${ENDPOINT} JSON Response
    ...
    IF  '${ENDPOINT}'=='Card Search'
        Should Be Equal As Strings  ${response.json()}[cardId]  ${mytestdata}[card_id]
