...
*** Test Cases ***
Click On Issuer Groups
    [Tags]  JIRA:BOT-1823  qTest:31156479  Regression  JIRA:BOT-1831
    [Setup]  Refresh Page
    [Teardown]  Run Keyword IF Test Failed  Refresh Page

    Click Configuration Manager Link  Issuer Groups
    Validate Correct Module Displays  issgrps  issuerGroups
...
#   CHECKING ALL THE WAYS POSSIBLE IF THE MODULE IS ON SCREEN
    ...
    Run Keyword And Ignore Error  Wait Until Element Is Visible  //div[@id="${module}_content"]//tr[@onclick]  timeout=30
    ${count}  Get Element Count  //div[@id="${module}_content"]//tr[@onclick]
    wait until done processing
    ${status}  run keyword and return status  element should not be visible  //div[@id="error_content"]
