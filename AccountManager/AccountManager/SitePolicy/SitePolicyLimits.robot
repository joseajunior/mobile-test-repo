...
Parkland - Successfull Add Gas - Plus
    [Tags]  JIRA:ROCKET-89  qTest:53560084  PI:11  checked
    [Setup]  Get Parkland Carrier
    Open Account Manager
    Search For ${parkland_carrier.member_id} And Go to Site Policies Tab
    Click On Submit For Site Policy Search
    Click On Site Policy ${sitepolicy}
...
Parkland - Successfull Add Gas - Regular
    [Tags]  JIRA:ROCKET-89  qTest:53560084  PI:11
    [Setup]  Get Parkland Carrier
    Open Account Manager
    Search For ${parkland_carrier.member_id} And Go to Site Policies Tab
    Click On Submit For Site Policy Search
...
Remove ${product} product if exists
    Click Element  //*[@id="policyProductsSearchContainer"]//button[@class="button searchSubmit" and contains(text(),"Submit")]
    ...
    Wait Until Element Is Visible  //*[@type="checkbox"]
...
Get Parkland Carrier
    ...
    ${parkland_carrier}  find carrier variable  select * from def_card where policy_type_id = 2 and id in ( select id from def_card where id between 2500001 and 2599999 and policy_type_id = 1 and id not in (2500052))  id
    ${sitepolicy}  query and strip  select ipolicy from def_card where policy_type_id = 2 and id = ${parkland_carrier.member_id}  db_instance=TCH
    set test variable  ${sitepolicy}
...
Verify Correct Products are added for Group
    [Arguments]  ${group}
    ${limitId}=  query and strip  select limit_id from def_lmts where limit_id = 'GAS' and carrier_id = ${parkland_carrier.member_id} and ipolicy = '${sitepolicy}'  db_instance=TCH
    should be equal as strings  ${limitId}  GAS
    ${defprodsql}  catenate  select prod_num from def_lmts_prod where carrier_id = ${parkland_carrier.member_id} and ipolicy = '${sitepolicy}' and prod_num in (select num from products_gas_grouping_desc) order by prod_num desc
    FOR  ${i}  IN RANGE  5
...
Click On Submit For Customer Search
    Click Element  //*[@id="customerSearchContainer"]//button[@class="button searchSubmit" and contains(text(),"Submit")]
    Wait Until Loading Spinners Are Gone
...
Click On Submit For Site Policy Search
    Click Button  //*[@id="customerSitePoliciesSearchContainer"]//button[@class="button searchSubmit" and contains(text(),"Submit")]
    Wait Until Loading Spinners Are Gone

Click On Site Policy ${num}
