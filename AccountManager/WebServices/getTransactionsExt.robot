...
Search Transactions Extended With Valid Date Ranges
    [Tags]  JIRA:BOT-1571  qTest:30704048  Regression  refactor
    [Documentation]  Input a valid data parameter and a valid response.
...
#   THAT'S WHY YOU NEED TO ADD +1 HOUR ON THE DB SQL
    ...
    ${Transactions}  getTransactionsExt  ${beginDate}  ${endDate}  ${beginTime}  ${endTime}
...
Search Transactions Extended Using Only Begin Date
    [Tags]  JIRA:BOT-1571  qTest:30704049  Regression
    [Documentation]  Insert only beginDate and expect an error.
    ${status}  Run Keyword And Return Status  getTransactionsExt  2019-03-24  ${empty}  00:00:00  ${empty}
    Should Not Be True  ${status}

Search Transactions Extended Using Only End Date
    [Tags]  JIRA:BOT-1571  qTest:30704050  Regression
    [Documentation]  Insert only endDate and expect an error.
    ${status}  Run Keyword And Return Status  getTransactionsExt  ${empty}  2019-03-24  ${empty}  00:00:00
    Should Not Be True  ${status}

Search Transactions Extended Using Null Parameters
    [Tags]  JIRA:BOT-1571  qTest:30704051  Regression
    [Documentation]  Insert null parameters and expect an error.
    ${status}  Run Keyword And Return Status  getTransactionsExt  ${empty}  ${empty}  ${empty}  ${empty}
    Should Not Be True  ${status}

Search Transactions Extended Using Date on Future
    [Tags]  JIRA:BOT-1571  qTest:30704052  Regression
    ...
    ${status}  Run Keyword And Return Status  getTransactionsExt  ${beginDate}  ${endDate}
    Should Not Be True  ${status}

Search Transactions Extended With a Typo On the Date
    [Tags]  JIRA:BOT-1571  qTest: 30704381  Regression
...
#   TYPO ON THE BEGIN DATE
    ${status}  Run Keyword And Return Status  getTransactionsExt  2019-03-2X  ${endDate}
    Should Not Be True  ${status}

    ${status}  Run Keyword And Return Status  getTransactionsExt  2019-?!-25  ${endDate}
    Should Not Be True  ${status}

    ${status}  Run Keyword And Return Status  getTransactionsExt  201@-03-25  ${endDate}
    Should Not Be True  ${status}

#   TYPO ON THE END DATE
    ${status}  Run Keyword And Return Status  getTransactionsExt  ${beginDate}  2019-03-2X
    Should Not Be True  ${status}

    ${status}  Run Keyword And Return Status  getTransactionsExt  ${beginDate}  2019-?!-25
    Should Not Be True  ${status}

    ${status}  Run Keyword And Return Status  getTransactionsExt  ${beginDate}  201@-03-25
    Should Not Be True  ${status}

Search Transactions Extended With Extra Character On the Time
    [Tags]  JIRA:BOT-1571  qTest:30704382  Regression
    [Documentation]  Validate that's not possible to get transactions searching by Begin Time when it has a typo.
    ${beginDate}  getDateTimeNow  %Y-%m-%d
    ${endDate}  getDateTimeNow  %Y-%m-%d  days=-1

#   EXTRA CHARACTER ON THE BEGIN TIME
    ${status}  Run Keyword And Return Status  getTransactionsExt  ${beginDate}  ${endDate}  000:59:59  00:00:00
    Should Not Be True  ${status}

    ${status}  Run Keyword And Return Status  getTransactionsExt  ${beginDate}  ${endDate}  00:599:00  00:00:00
    Should Not Be True  ${status}

    ${status}  Run Keyword And Return Status  getTransactionsExt  ${beginDate}  ${endDate}  00:59:599  00:00:00
    Should Not Be True  ${status}

#   EXTRA CHARACTER ON THE END TIME

    ${status}  Run Keyword And Return Status  getTransactionsExt  ${beginDate}  ${endDate}  00:00:00  233:59:59
    Should Not Be True  ${status}

    ${status}  Run Keyword And Return Status  getTransactionsExt  ${beginDate}  ${endDate}  00:00:00  23:599:59
    Should Not Be True  ${status}

    ${status}  Run Keyword And Return Status  getTransactionsExt  ${beginDate}  ${endDate}  00:00:00 23:59:599
    Should Not Be True  ${status}

Search Transactions Extended With Extra Character On the Date
    [Tags]  JIRA:BOT-1571  qTest:30704383  Regression
...
#   EXTRA CHARACTER ON THE BEGIN DATE
    ${status}  Run Keyword And Return Status  getTransactionsExt  20190-03-25  ${endDate}
    Should Not Be True  ${status}

    ${status}  Run Keyword And Return Status  getTransactionsExt  2019-003-25  ${endDate}
    Should Not Be True  ${status}

    ${status}  Run Keyword And Return Status  getTransactionsExt  2019-03-250  ${endDate}
    Should Not Be True  ${status}

#   EXTRA CHARACTER ON THE END DATE
    ${status}  Run Keyword And Return Status  getTransactionsExt  ${beginDate}  20190-03-25
    Should Not Be True  ${status}

    ${status}  Run Keyword And Return Status  getTransactionsExt  ${beginDate}  2019-003-25
    Should Not Be True  ${status}

    ${status}  Run Keyword And Return Status  getTransactionsExt  ${beginDate}  2019-03-250
    Should Not Be True  ${status}

Search Transactions Extended With Invalid Date
    [Tags]  JIRA:BOT-1571  qTest:30704384  Regression
    [Documentation]  Validate that's not possible to get transactions searching by Invalid Begin Date
    ${BeginDate}  getDateTimeNow  %Y-%m-%d
    ${EndDate}  getDateTimeNow  %Y-%m-%d

#   INVALID BEGIN DATE
    ${status}  Run Keyword And Return Status  getTransactionsExt  2019-03-65  ${EndDate}
    Should Not Be True  ${status}

    ${status}  Run Keyword And Return Status  getTransactionsExt  2019-15-26  ${EndDate}
    Should Not Be True  ${status}

    ${status}  Run Keyword And Return Status  getTransactionsExt  0000-03-10  ${EndDate}
    Should Not Be True  ${status}

#INVALID END DATE
    ${status}  Run Keyword And Return Status  getTransactionsExt  ${BeginDate}  2019-03-65
    Should Not Be True  ${status}

    ${status}  Run Keyword And Return Status  getTransactionsExt  ${BeginDate}  2019-15-26
    Should Not Be True  ${status}

    ${status}  Run Keyword And Return Status  getTransactionsExt  ${BeginDate}  00193-03-10
    Should Not Be True  ${status}

Search Transactions Extended With Incomplete Dates
    [Tags]  JIRA:BOT-1571  qTest:30704385  Regression
    [Documentation]  Validate that's not possible to get transactions searching by an incomplete date format

#   INCOMPLETE BEGIN DATE
    ${status}  Run Keyword And Return Status  getTransactionsExt  201-03-25  2019-03-10
    Should Not Be True  ${status}

    ${status}  Run Keyword And Return Status  getTransactionsExt  2019-3-25  2019-03-10
    Should Not Be True  ${status}

    ${status}  Run Keyword And Return Status  getTransactionsExt  2019-03-5  2019-03-10
    Should Not Be True  ${status}

#   INCOMPLETE END DATE
    ${status}  Run Keyword And Return Status  getTransactionsExt  2019-03-10  201-03-25
    Should Not Be True  ${status}

    ${status}  Run Keyword And Return Status  getTransactionsExt  2019-03-10  2019-3-10
    Should Not Be True  ${status}

    ${status}  Run Keyword And Return Status  getTransactionsExt  2019-03-10  2019-03-0
    Should Not Be True  ${status}

Search Transactions Extended With Incomplete Times
    [Tags]  JIRA:BOT-1571  qTest:30704386  Regression
    [Documentation]  Validate that's not possible to get transactions searching by an incomplete date format
    ${beginDate}  getDateTimeNow  %Y-%m-%d
    ${endDate}  getDateTimeNow  %Y-%m-%d

#   INCOMPLETE BEGIN TIME
    ${status}  Run Keyword And Return Status  getTransactionsExt  ${beginDate}  ${endDate}  2:59:59  00:00:00
    Should Not Be True  ${status}

    ${status}  Run Keyword And Return Status  getTransactionsExt  ${beginDate}  ${endDate}  23:5:59  00:00:00
    Should Not Be True  ${status}

    ${status}  Run Keyword And Return Status  getTransactionsExt  ${beginDate}  ${endDate}  23:59:4  00:00:00
    Should Not Be True  ${status}

#   INCOMPLETE END TIME
    ${status}  Run Keyword And Return Status  getTransactionsExt  ${beginDate}  ${endDate}  00:00:00  2:59:59
    Should Not Be True  ${status}

    ${status}  Run Keyword And Return Status  getTransactionsExt  ${beginDate}  ${endDate}  00:00:00  23:5:59
    Should Not Be True  ${status}

    ${status}  Run Keyword And Return Status  getTransactionsExt  ${beginDate}  ${endDate}  00:00:00  23:59:0
    Should Not Be True  ${status}
